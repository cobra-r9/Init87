...
> When you ask **why?**, it will lead you to a wonderfull location - the location is the **glibc.so** that is a `C` shared library.
...
> It is good to note that : `cat /usr/include/stdio.h | grep "int printf"` will return you the function declaration **and not the definition** of the printf function. So, more accurately, the stdio.h only tells the compiler that - **hey : this is how the printf's signature looks like.** and the linux dynamic linker (ld-linux.so) will search for the printf in the glibc.so (GNU C standard library) and fill the holes.

## But which holes?

Not definitely an $$shole, but the functions : as I said earlier, the assembler will produce an object file : that object file will have an unresolved symbol - printf (ie, a function with unknown definition, but has a signature) - this is the hole in object file produced by assembler. Now, the linux dynamic linker (`ld-linux.so`) will fetch these holes : link the functions to their definitons from the linux standard library - which is a precompiled shared library glibc.so.6 (or whatever depending on your OS).
